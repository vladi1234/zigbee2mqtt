...
class HomeAssistant extends Extension {
    ...
    getConfigs(resolvedEntity) {
        if (resolvedEntity.type === 'group') {
            return [];
        } else {
            // resolvedEntity.type === 'device'
            if (!resolvedEntity || !resolvedEntity.definition || !this.mapping[resolvedEntity.definition.model]) {
                return [];
            }
            ...
            if (resolvedEntity.settings.homeassistant) {
                ...
                configs = configs.filter((config) =>
                    !s.hasOwnProperty(config.object_id) || s[config.object_id] != null);
